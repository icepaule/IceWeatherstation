...
- Die Netzliste (welcher Pin mit welchem verbunden ist) wurde direkt aus der obigen Tabelle erzeugt. **Update 2026-07-19 (1):** Pin-Positionen direkt aus den SVG-Koordinaten jedes Bauteils berechnet (Transformationskette pro Connector aufgelöst, korrekt für Rotation/Skalierung/mm-vs-Zoll) — Wires liegen exakt auf den Anschluss-Pins.
- **Update 2026-07-19 (2), kreuzungsfreies Routing:** Statt jedes Bauteil direkt sternförmig mit dem ESP32 zu verbinden (was zwangsläufig zu einem Wire-Gewirr an einem Punkt führt, siehe Screenshot-Beispiel BME280), sind gemeinsame Signale jetzt als **Bus-Ketten** verlegt: 3,3V/GND/SDA/SCL laufen als eine Leitung nacheinander durch alle Bauteile, die sie brauchen (ESP32 → BME280 → AS3935 → OLED usw.), statt N-mal einzeln zum ESP32 zurück. Die vier Sammelleitungen (3,3V, SDA, SCL, AS3935-INT) sind zusätzlich als eigene, klar getrennte "Lanes" **oberhalb** der gesamten Schaltung verlegt (jede auf eigener Höhe mit kleinem X-Versatz), beide GND-Ketten (links/rechts) laufen unterhalb — dadurch kreuzt keine Leitung eine andere. Bauteile sind passend zur Pin-Reihenfolge am ESP32 (links: GPIO34/35/27/14+GND, rechts: SDA/SCL/GPIO4/GPIO2+GND) in zwei Spalten links/rechts vom ESP32 angeordnet. Einzige technisch unvermeidbare Ausnahme: `AS3935 INT` muss auf `GPIO25` (linke Pinreihe), obwohl AS3935 selbst wegen des I2C-Busses rechts sitzt — dafür bekommt genau diese eine Leitung ihre eigene, höchste Lane, damit sie nichts anderes kreuzt.
- **Update 2026-07-19 (3), echte Fritzing-Verifikation + Schematic/Code:** Ab hier nicht mehr blind gebaut — Fritzing 1.0.1 + Parts-Datenbank lokal installiert und die Datei per Xvfb (virtueller X-Server) tatsächlich in der echten App geöffnet und screenshot-geprüft, statt nur die XML-Struktur zu validieren. Dabei zwei echte Fehler gefunden und live gefixt: (a) eine verbliebene Kreuzung zwischen Windfahne, ihrem Spannungsteiler-Widerstand und dem SEN0232 (Widerstand saß versetzt statt in der Signalspalte); (b) bestätigt, dass Fritzing die eigentliche Verkabelung **pro Ansicht getrennt** speichert — Verbindungen aus der Breadboard-Ansicht erscheinen NICHT automatisch als Ratsnest in Schematic/PCB (mehrfach getestet, auch mit gespiegelten `connects`-Einträgen — hat nicht funktioniert). **Schematic-Ansicht** zeigt daher alle 13 Bauteile mit korrekten Schaltzeichen in sauberem Raster, aber **ohne** Verbindungslinien — die müssten in Fritzing selbst nachgezogen werden (Status "No connections to route" beim Öffnen ist erwartet, kein Fehler). Gleiches gilt für die PCB-Ansicht (nur Footprint-Platzierung, keine Traces).
- **Code-Tab befüllt:** Fritzings eingebauter "Code"-Tab (normalerweise für Arduino-Sketches) enthält jetzt zwei Dateien als Referenz: `IceWeatherstation_TasmotaBacklog.ino` (= `firmware/config/backlog.txt`, die Tasmota-Konsolen-Befehle) und `IceWeatherstation_autoexec.ino` (= `firmware/berry/autoexec.be`, das Dashboard-Skript) — beide sind **kein** kompilierbarer Arduino-Code, nur mit `.ino`-Endung gespeichert, weil Fritzings Code-Tab keine andere Dateiendung zulässt. Einbettung live verifiziert: beim Öffnen in einer frischen Fritzing-Instanz (andere Prozess-ID, simuliert einen fremden Rechner) zeigt die Statuszeile korrekt „File … was restored from the .fzz file" — die Dateien sind also wirklich im `.fzz` eingebettet, nicht nur über einen lokalen Pfad verlinkt.
- ⚠️ **Fallstrick, live gefunden:** Öffnet man die `.fzz` in Fritzing und speichert dort einmal ab, entfernt Fritzing das eingebettete ESP32-Bauteil automatisch aus der Zip-Datei (es wird beim ersten Laden in Fritzings eigene lokale Parts-Datenbank importiert und gilt danach als "lokal vorhanden", nicht mehr bundle-pflichtig) — das **zerstört die Portabilität** auf einem dritten Rechner. Für Bearbeitungen daher: entweder das ESP32-Bauteil einmal dauerhaft in die eigene Fritzing-Installation importieren (dann ist es kein Problem mehr), oder nach jeder Bearbeitung prüfen, ob `part.ESP32_DevKitc_V4_1.fzp` + die 3 zugehörigen SVGs noch in der `.fzz` (als ZIP) enthalten sind.
